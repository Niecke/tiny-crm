...
|---|---|---|---|
| Contact | name, job_title, email + email_secondary, phone + phone_secondary, website, street / postal_code / city / country, lifecycle_status, relation_type, source, preferred_language, birthday, known_day_rate + rate_currency, works_with_freelancers *(null = never asked)*, tags, notes | → organization (FK), ← interactions (M2M), ← projects (M2M), ← documents (M2M) | CRUD, `?search` on name, `?organization_id`, `?lifecycle_status`, `?relation_type`, `?source`, `?country`, `?works_with_freelancers=yes\|no\|unknown` |
...
| Interaction | kind (call/meeting/email/note/other), subject, notes, occurred_at, duration_minutes, done, tags | M2M contacts, organizations, deals, projects | CRUD, filters: search, contact_id, organization_id, deal_id, project_id, kind, upcoming |
...
- [x] **T10 · P0 · Backups with a rehearsed restore**
      *Done:* two independent layers. Hetzner server snapshots (`infrastructure: terraform/hetzner/tiny-crm.tf`, seven rolling) cover losing the node; a daily CronJob (`charts/tinycrm/templates/backup-cronjob.yaml`, image built from `backup/`) covers everything else. One run writes one `tiny-crm-<TS>.tar` — a brotli'd `pg_dump` plus the raw MinIO documents — to `gs://niecke-it-tiny-crm-backups` through its S3 endpoint, using the HMAC key that already existed, so no GCP key file is involved.
      **Restored 2026-09-05** into a throwaway kind cluster: row counts matched production, object count exceeded the `documents` rows as expected from previews, login through `/docs` returned a token and a document's content came back. Procedure in `infrastructure: scripts/niecke-it/RESTORE.md`.
      Sub-item 3 closed by the migration — the archive now lives at a different provider from the workload, so losing either does not lose both. **Sub-item 2 is still open:** a failed run is silent. Recorded as the first known gap in `BACKUP.md`, with a monthly freshness check in `niecke-it-consulting: MAINTENANCE.md` as the interim.

- [x] **T11 · P1 · Deployment in the repository**
      *Done:* Flux, pull-based. `deploy/flux/` holds the GitRepository, the Kustomization that applies it, and the HelmRelease; the chart is `charts/tinycrm/`. Merging to `main` is the deploy — `promote.yml` retags the digest that passed the integration test and commits the sha, and the cluster pulls it. No cluster credentials in GitHub, and the k3s API is not published.
      Rollback is `helm -n tinycrm rollback tinycrm`; a failed upgrade rolls itself back after three retries without help. `deploy/README.md`.
...
- [x] **T16 · P1 · Attach documents and interactions to any record**
      ...
      *Done:* documents now attach to **contacts, organizations, deals and projects**; interactions to **contacts, organizations, deals and projects**. Six new join tables (documents gained contacts/organizations/deals, interactions gained organizations/deals/projects; `project_documents` and `interaction_contacts` already existed). Every link is many-to-many in both directions — one framework agreement covers two deals, and the same NDA is filed against both the person and their company without being uploaded twice. Reads carry the id lists; `?contact_id=`, `?organization_id=`, `?deal_id=`, `?project_id=` on both list endpoints answer "everything filed against this record" and "every call about this deal".
      **Six real tables, not one polymorphic `attachments(target_type, target_id)`.** A polymorphic target cannot carry a foreign key, so deleting a deal would leave rows pointing at nothing and every read would need a defensive existence check. `CASCADE` on both sides of each link instead: removing either end drops the *link row* and never the document or interaction at the other end. Verified in SQL — delete the deal, the join row goes, the file stays.
      *Load-bearing detail:* every id in every link list goes through one shared `load_scoped()` (`app/links.py`, extracted from the project router so all three routers resolve links identically). A foreign key alone would accept another tenant's id, and since these reads send linked records back out, that is a data leak rather than untidy data. A miss is a **404, not a silent drop** — quietly ignoring an id would leave the operator believing a contract is filed against a deal when it is not.
      **Upload resolves its links before the body reaches S3.** Doing it the other way round would leave an object in the bucket with no row that could ever delete it; there is a test asserting the object store is untouched after a refused upload. Multipart has no list type, so the link lists arrive as JSON arrays in form fields, exactly like `tags`, and a malformed one is a 422 rather than a 500.
      *Migration `j0k1l2m3n4o5`* creates the six tables; nothing to backfill and the two pre-existing link tables are untouched. The composite primary key is the pair, so the same document cannot be attached to the same deal twice. Verified up, down and up again with linked rows present, and `alembic check` reports no drift.
      *Tests:* `backend/tests/test_attachments.py` (27 cases: filing against all four kinds, no-link documents, one document under several records, add/replace/clear, an untouched list surviving a PATCH, parametrised unknown-id refusals, cross-tenant refusals, the no-orphan-in-S3 check, malformed lists, all filters incl. two at once narrowing rather than widening, CASCADE keeping the document, and `project_documents` still agreeing when written from either side), plus `frontend/test/attachments_test.dart` (12 cases). `ci/smoke.sh` files a document against its deal at upload time, filters by it, refuses a bad link, then deletes the deal and asserts the document survived detached.
      *UI:* one reusable `RecordPicker` (chips + a pick dialog, generalised from the interaction form's contact picker) and an `AttachmentPickers` set of four, used by the document upload dialog, the document metadata dialog and the interaction form — eight hand-rolled pickers would have drifted apart. Detail screens grew `AttachedDocumentsSection` / `AttachedInteractionsSection`: contact gets documents, organization and deal get both, project gets interactions. Logging or filing from a detail screen pre-links the record it was opened from.
      *Deliberately not included:* attaching to tasks. A task is a to-do, not a record you file paperwork against — and T14 already gives the useful direction, a task pointing at the interaction it came out of.
      *Also unblocks:* T38 — a tender is mostly a pile of PDFs.

- [x] **T17 · P1 · Contact fields a business actually files**
      A contact held name, email, phone, one free-text `address`, tags and notes — not enough to write a letter, judge whether an approach is worth making, or tell a customer from a partner.
      *Done:* 16 columns on `contacts` in one migration. The address book half — `job_title`, `email_secondary`, `phone_secondary`, `website`, `lifecycle_status` (lead / prospect / customer / former), `source` (referral / inbound / outbound / event / job_board / tender_portal / other), `preferred_language`, `birthday` — plus the freelance half: `relation_type` (customer / partner / subcontracting_target / contracting_authority), `known_day_rate` + `rate_currency`, and `works_with_freelancers`. Closed sets are `String` columns constrained by Pydantic `Literal`s, the same choice as `Deal.stage`: adding a value must not become an `ALTER TYPE`. `source` reuses T40's watch kinds, because a sweep that becomes a person is how those contacts arrive.
      **`address` is now four columns** — `street`, `postal_code`, `city`, `country` — so it can feed a letter, an invoice or a vCard. `country` is ISO 3166-1 alpha-2 and `preferred_language` ISO 639-1, normalised by the schema (`to_upper` / `to_lower`) so a filter cannot split "AT", "at" and "Austria" into three countries.
      *Load-bearing detail:* **the migration does not parse the old address.** The whole previous value moves into `street` verbatim; splitting "Hauptstraße 1, 1010 Wien" by guessing where the postcode starts is how a CRM posts an invoice to the wrong place, and the operator can move the parts across in seconds with the data in front of them. The downgrade rejoins the parts with `concat_ws` and `nullif`, so a contact that never had an address does not come back with an empty string.
      **Status and type stay two columns**, as specified: type is *what this party is to me*, status is *how far along we are*. Both are filterable and they narrow together, so "partners we have not approached yet" — the row that a single collapsed column would lose — is one request.
      **`works_with_freelancers` is nullable, and the filter is tri-state.** `?works_with_freelancers=unknown` is the whole point: a plain bool parameter could ask "yes" and "no" but not "never asked", and never-asked is the list that produces the next approach. `no` and `unknown` are different answers everywhere — column, filter, form dropdown and the chip on contact detail (grey for "they don't", not red: red is already the overdue colour).
      *`known_day_rate` + `rate_currency` are a pair*, refused half-way in either direction — the same rule T13 applies to a volume with no unit, since "800" is not a rate and "EUR" is not a number. The check runs on the **merged** row on PATCH, so setting a rate on a contact with no currency fails even though the request never mentions one; clearing the rate clears the currency with it, while sending a currency alongside that clear still fails. Money is `Numeric(14, 2)` and a decimal string end to end, never a float. `Currency` and `Money` moved to `app/schemas/common.py` so a contact's rate and a deal's rate cannot drift apart.
      *Filterable from day one:* `?lifecycle_status=`, `?relation_type=`, `?source=`, `?country=`, `?works_with_freelancers=yes|no|unknown`. An unknown filter value is a 422 rather than being ignored — silently dropping it would show every contact and look like an answer. `lifecycle_status` and `relation_type` carry indexes like `Deal.stage`; the rest wait for T31.
      *Migration `l2m3n4o5p6q7`* adds the columns and the two indexes, backfills `street`, then drops `address`. Reversible both ways.
      *Tests:* `backend/tests/test_contact_fields.py` (25 cases: the full round trip, an empty contact leaving all 16 unset, normalisation, seven refused values, both halves of the rate rule on create and on PATCH incl. the clear-clears-currency and the explicit-contradiction cases, an unrelated PATCH leaving the rate alone, the three freelancer states and all three filter answers, status/type independence, two filters narrowing, source and country filters, and refused filter values), plus `frontend/test/contact_fields_test.dart` (8 cases incl. unknown enum values reading as unset rather than throwing). `ci/smoke.sh` files a contact with the business fields, asserts the country and the rate read back exactly, runs the two-filter and never-asked queries, and checks half a rate is refused.
      *UI:* the contact form grew Address, Relationship and Filing sections — status, type, source and a **three-way** "Works with freelancers?" dropdown that starts at "Never asked", a rate + currency pair the form cannot leave half-filled, and a birthday picker that refuses the future. Contact detail leads with status / type / freelancer chips and shows the address as an envelope. The dashboard contacts panel has three filter dropdowns (status, type, freelancers) that combine, and its tile now reads "Head of Delivery · ACME".
      *Does not include:* T36's channel-compliance fields. The suggested order pairs them into one migration; this shipped first, so T36 is its own `ALTER TABLE` — still cheap, still P0.
